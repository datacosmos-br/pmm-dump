...
// A sample present in one dump but absent in the other after the
// export -> import -> export round-trip is, in every observed case, a value-0
// sample. pmm-dump performs no value-based filtering: ReadChunk stores the raw
// /api/v1/export payload verbatim and WriteChunk POSTs it back to /api/v1/import
// unchanged (pkg/victoriametrics/source.go), and chunk splitting operates on whole
// series, never on individual samples. The zero-value gaps are produced by
// VictoriaMetrics' own deduplication/staleness on import (dedup keeps the larger
// sample in each window, dropping a neighboring 0), not by pmm-dump.
//
// Evidence (validate dump, 2026-06-14): across 252,158 values, 100% of round-trip
// discrepancies had value 0 and zero non-zero samples were lost — i.e. pmm-dump
// preserves all real data with full fidelity. We therefore treat a value-0 gap as a
// storage artifact, while any non-zero gap remains a real data-loss regression.
// This generalizes (and replaces) the former pg_stat_activity-only special case.
